...
•✅ 音频转字幕：上传任意音频，自动生成带时间戳的 SRT 字幕，支持多语种识别。
•✅ 视频转字幕：从视频中提取音频并识别，支持软/硬字幕嵌入。
•✅ 批量处理：一次性处理多个音频文件，自动保存结果。
•✅ 字幕自动打轴（强制对齐）：你提供正确的稿子文本，系统自动将稿子精准对齐到音频时间轴，生成文字完全正确、时间轴准确的字幕。特别适合已经拥有准确文稿的配音、课程、有声书等场景。


-  **语音识别转字幕** – 基于 faster-whisper，支持 GPU 加速，输出 SRT/JSON/TXT。
-  **字幕自动打轴（强制对齐）** – 你提供正确的稿子文本，系统自动将稿子精准对齐到音频时间轴，生成文字完全正确、时间轴准确的字幕。
...
> 注意：语音识别首次使用需下载模型（约 1.5~5 GB），请保持网络畅通。
...
字幕自动打轴（强制对齐）功能说明
此功能允许你用自己准备的稿子文本，直接生成与音频时间轴完全对齐的字幕。特别适合已经拥有准确文稿的配音、课程、有声书、古诗词朗诵等场景。中文场景下不如fireredASR2S！！！
使用方法
1.	切换到 “字幕自动打轴 (文稿生字幕)” 标签页。
2.	上传音频文件（WAV、MP3 等）。
3.	在文本框中粘贴你已校对好的稿子。
        
关键：段落之间请用空行（连续两个换行符）分隔，例如：
...
4.	勾选 “根据空行断句”（推荐），其他合并参数可保持默认。
5.	点击 “生成精准字幕”，稍等片刻，即可在 “整句 SRT (按空行)” 标签页中获得文字100%正确、时间轴精准的字幕。

⚠️ 字幕自动打轴的已知缺点（请务必阅读）
尽管该功能在多数场景下表现良好，但受限于底层算法及开源生态的现状，存在以下局限性。请评估你的需求是否匹配：
1. 对稿子文本格式要求极高
•	必须与音频读音完全一致：稿子中的每个字、标点符号（包括语气词、错别字）都应与音频中的发音严格对应。任何差异（例如“的”vs“地”、“了”vs“啦”）都可能导致对齐失败或时间戳错乱。
•	对换行符敏感：目前仅支持连续两个换行符作为段落分隔。单个换行、空格、制表符均不能正确识别为段落边界。如果你习惯用单个换行分段，生成的字幕可能不会被合并。

缺乏“模糊匹配”能力
•	与一些商业级或工业级对齐工具（如 FireRedASR 的强制对齐）不同，本工具采用精确字符匹配算法。若稿子与音频存在个别字词差异（如同音错字、多字漏字），算法无法智能容错，可能导致整段字幕时间轴错位。
•	解决方案：请务必提前校对稿子，确保与音频读音完全一致。

依赖外部组件稳定性
本功能依赖于 faster-whisper、transformers 等开源库，这些库的版本更新可能导致依赖冲突或行为变化。整合包已锁定稳定版本，但若你自行升级，可能引入新问题。

输出字幕样式有限
目前仅支持标准 SRT 格式（整句字幕）。如需卡拉 OK 效果（单词级逐字高亮）或 ASS 高级样式，需要二次开发。
...
https://hf-mirror.com/jonatasgrosman/models

ASR模型下载地址：
https://hf-mirror.com/jonatasgrosman/models
https://hf-mirror.com/collections/Systran/faster-whisper
