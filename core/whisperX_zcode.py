...
# -*- coding: utf-8 -*-
"""
WhisperX 语音识别独立增强版（参数全开放 / 最终修复版）
- 音频识别 / 视频字幕 / 批量处理
- 可选精细时间戳对齐
- 断句控制（时长/字符/标点/静音，可独立组合）
- VAD 高级参数、界面截断保护、一键打开输出目录
- 长音频自动切分（超 30 分钟按 10 分钟分段转写后回拼时间戳）
- 批量处理双缓冲流水线（CPU 预处理与 GPU 推理重叠）

适配环境: gradio 5.x / faster-whisper 1.1.0 / whisperx 3.3.1 / torch 2.5.1

【最终版变更（相对上一版）】
  A. 修复批量流水线竞态：临时文件按任务隔离（sink 机制），当前文件清理时
     不再误删后台已预处理的下一个文件音频（原实现从第 2 个文件起必失败）
  B. 修复长音频分段偏移：改为 idx * 段长（片段真实起点），删除引用未定义
     变量 part / segment_minutes 的错误代码；原实现用"上一段末句结束时间"
     作偏移，忽略片段尾部静音，时间戳逐段累积前移且可能与上一段重叠
  C. 修复分段进度条：gr.Progress 没有 update 方法（原 hasattr 判断恒为
     False，进度从不更新），改为 callable 判断 + 子区间映射
     （progress_base / progress_span），进度不再回跳冲突
  D. FFmpeg 切分改输入端快进（-ss 移到 -i 之前），长文件切分从
     O(n²) 解码降为 O(n)；主流程输入为 PCM WAV，seek 采样级精确
  E. 长音频分段时自动跳过 wav2vec2 精细对齐（对齐需整段加载音频，
     长音频易 OOM），结果中附带提示
  F. 分段临时文件名加 uuid 防撞名；切分失败自动回退整文件推理
  G. 批量循环移除 per-file torch.cuda.empty_cache()（避免缓存块反复
     释放重分配拖慢流水线）
  H. 片段转写失败计数透出（split_used / total_parts / failed_parts），
     用户可在结果中看到缺失警告；全部片段失败时报错而非返回空结果
  I. 移除死代码（processed_count / force_no_split）；VAD/断句参数在
     循环外预构建，减少重复开销
...
# ==================== 长音频切分配置 ====================
LONG_AUDIO_THRESHOLD_MINUTES = 30   # 超过此时长触发自动分段
LONG_AUDIO_SEGMENT_MINUTES = 10     # 每段时长
...
# 查找 ffprobe
if sys.platform == "win32":
    FFPROBE_PATH = str(PORTABLE_FFMPEG_DIR / "ffprobe.exe") if (PORTABLE_FFMPEG_DIR / "ffprobe.exe").exists() else shutil.which("ffprobe")
else:
    FFPROBE_PATH = str(PORTABLE_FFMPEG_DIR / "ffprobe") if (PORTABLE_FFMPEG_DIR / "ffprobe").exists() else shutil.which("ffprobe")
if not FFPROBE_PATH:
...
def normalize_file_input(f):
    """统一解析 Gradio 文件输入（FileInfo / dict / str / Path / tuple），返回 (路径, 原始文件名)。"""
    if f is None:
        return None, None
    ...
    if isinstance(f, (list, tuple)):
        ...
        return None, None
    if hasattr(f, 'orig_name') or hasattr(f, 'path'):  # Gradio 4/5 FileInfo
...
def safe_text(text: str, max_len: int = None) -> str:
    """对过长输出截断；SRT 按最后一个完整空行回退截断，避免破坏条目结构。"""
    if max_len is None:
        with config_lock:
...
def split_long_segments(segments, enable_duration_split=False, max_duration=None,
    ...
    for seg in segments:
        ...
        for sub_start, sub_end in sub_segments_times:
            ...
            sub_words = [w for w in words
                         if w.get('start', 0) >= sub_start and w.get('end', 0) <= sub_end + 1e-4]
                ...
                if current.strip():
                    ...
                    if words:
                        for w in words:
                            ...
                            if sub_start - 1e-4 <= w_start < sub_end + 1e-4:
                                sub_words.append(w)
...
def _split_by_time_and_chars(text, start, end, words,
    ...
    if words and len(words) > 1:
        ...
        for i in range(0, len(words), words_per_seg):
            ...
            result.append({'start': sub_start, 'end': sub_end, 'text': sub_text, 'words': chunk_words})
        return result
    else:
        ...
        for i in range(num_splits):
            ...
            if sub_text:
                result.append({'start': sub_start, 'end': sub_end, 'text': sub_text, 'words': []})
        return result

_CJK_RANGE = r'\u4e00-\u9fff\u3040-\u30ff\uac00-\ud7af'

def clean_cjk_spaces(text: str) -> str:
    """只删除 CJK 字符之间的空格，保留中英混排时英文单词间的空格。"""
    if re.search(rf'[{_CJK_RANGE}]', text):
...
def get_system_info():
    ...
    info.append(f"长音频切分: 已启用 (>{LONG_AUDIO_THRESHOLD_MINUTES}分钟自动分段, "
                f"每段{LONG_AUDIO_SEGMENT_MINUTES}分钟, 分段时自动跳过精细对齐)")
    return "\n".join(info)
...
def match_local_align_by_lang(lang, local_align):
    """先边界匹配语言代码，再按语言全名匹配；避免 'en' 子串误命中 'french'。"""
...
class WhisperXManager:
    ...
    def apply_whisperx_align(self, result, audio_path, language, device, model_choice):
        ...
        try:
            ...
            effective_lang = language or detected
            if model_choice == "auto":
                align_model_path = match_local_align_by_lang(effective_lang, local_align)
    ...
    def _prepare_audio(self, audio_input, force_preprocess=False, sink=None):
        """返回用于转写的音频路径（16kHz mono）。
        sink: 任务级临时文件登记表（批量流水线用它隔离文件归属，
        避免清理当前文件时误删后台已预处理的下一个文件）；缺省登记到全局列表。"""
    ...
    def get_audio_duration(self, audio_path):
        """获取音频时长（秒），使用 soundfile 或 ffprobe。"""
        try:
    ...
    def _split_audio_by_duration(self, audio_path, total_duration,
                                 segment_minutes=LONG_AUDIO_SEGMENT_MINUTES, sink=None):
        """将长音频切分为临时小段（16kHz 单声道 WAV），返回临时文件路径列表。

        - -ss 位于 -i 之前：输入端快进，避免逐段解码丢弃前面全部音频（O(n²) I/O）；
          主流程输入多为 PCM WAV，输入 seek 为采样级精确；
        - 临时文件登记到 sink（任务级隔离），避免批量流水线误删；
        - 任意一段失败则清理已生成文件并返回 [audio_path]，回退整文件推理。
        """
        if sink is None:
            sink = self.temp_files
        ...
        for i in range(num_segments):
            temp_file = CACHE_DIR / f"{base_name}_{uid}_part_{i:03d}.wav"
            cmd = [
                FFMPEG_PATH, "-y",
                "-ss", str(i * segment_duration),   # 输入端快进（在 -i 之前）
                "-i", audio_path,
            ...
            try:
                ...
                logger.error(f"音频切分失败 (第 {i + 1} 段): {e}")
                for f in temp_files:
                    try:
                        os.unlink(f)
                    except Exception:
    ...
    def transcribe_with_segments(self, audio_path, language=None, beam_size=5,
                                 ...
                                 progress=None, progress_base=0.0, progress_span=1.0,
                                 sink=None):
        """统一转写入口。

        - 短音频（< LONG_AUDIO_THRESHOLD_MINUTES）直接整段推理；
        - 长音频自动切分为 LONG_AUDIO_SEGMENT_MINUTES 片段，分段转写后
          按「片段真实起点 = idx * 段长」回拼全局时间戳；
        - 返回；分段结果附加 split_used / total_parts / failed_parts 标记。
        - progress: 可选 gr.Progress，映射到 [progress_base,
          progress_base + progress_span] 子区间，避免与外层进度冲突。
        """
        total_duration = self.get_audio_duration(audio_path)
        if total_duration is None or total_duration < LONG_AUDIO_THRESHOLD_MINUTES * 60:
            return self.transcribe(audio_path, language, beam_size, vad_filter,
                                   vad_parameters, word_timestamps, initial_prompt)

        part_files = self._split_audio_by_duration(
            audio_path, total_duration,
            segment_minutes=LONG_AUDIO_SEGMENT_MINUTES, sink=sink)
        if len(part_files) == 1 and part_files[0] == audio_path:
            # 切分失败，回退整文件推理
            return self.transcribe(audio_path, language, beam_size, vad_filter,
                                   vad_parameters, word_timestamps, initial_prompt)

        segment_duration = LONG_AUDIO_SEGMENT_MINUTES * 60
        total_parts = len(part_files)
        full_segments, full_words = [], []
        current_lang, max_prob = None, 0.0
        failed_parts = 0

        for idx, part_path in enumerate(part_files):
            # 进度：映射到外层进度子区间（gr.Progress 可调用，无 update 方法）
            if progress is not None and callable(progress):
                progress(progress_base + progress_span * (idx + 1) / total_parts,
                         desc=f"转写片段 {idx + 1}/{total_parts}")
            # 关键修复：偏移 = 片段真实起点（固定值 idx * 段长）。
            # 原实现用上一段末句结束时间作偏移，忽略片段尾部静音，
            # 误差逐段累积且引用了未定义变量 part / segment_minutes。
            offset = idx * segment_duration
            result, err = self.transcribe(part_path, language, beam_size, vad_filter,
                                          vad_parameters, word_timestamps, initial_prompt)
            if err or not result:
                logger.warning(f"分段 {idx + 1}/{total_parts} ({part_path}) 转写失败: {err}")
                failed_parts += 1
                continue
            for seg in result.get("segments", []):
                ...
                for w in seg.get("words", []):
                    w["start"] += offset
                    w["end"] += offset
            full_segments.extend(result.get("segments", []))
            full_words.extend(result.get("words", []))
            if result.get("language_probability", 0) > max_prob:
                max_prob = result["language_probability"]
                current_lang = result.get("language", "en")
            del result
            gc.collect()

        if failed_parts == total_parts:
            return None, "所有片段转写失败，请检查音频内容或调整 VAD 设置后重试"

        merged = {
            "language": current_lang or "en",
            "language_probability": max_prob,
...
# ==================== 转录函数 ====================
def ensure_model_loaded(model_size, device, compute_type):
    success, msg = manager.load_asr_model(model_size, device, compute_type)
    if not success:
...
def _norm_lang(language):
    """支持下拉 auto（None 交给 faster-whisper 自动检测）。"""
    return None if (language is None or str(language).strip().lower() in ("", "auto")) else str(language).strip()

def _clean_sink(sink):
    """清理任务级临时文件登记表中的所有文件。"""
    if not sink:
        return
    for f in list(dict.fromkeys(sink)):  # 去重且保序
        try:
            os.unlink(f)
        except Exception:
            pass
...
def _build_split_notes(result, enable_align):
    """生成长音频分段相关的提示信息（供三个转写入口共用）。"""
    if not isinstance(result, dict) or not result.get("split_used"):
        return ""
    notes = [f"长音频已自动分段转写（{result.get('total_parts', '?')} 段 × {LONG_AUDIO_SEGMENT_MINUTES} 分钟）"]
    if enable_align:
        notes.append("精细对齐已自动跳过（wav2vec2 需整段加载音频，长音频易致显存溢出）")
    if result.get("failed_parts"):
...
def transcribe_audio(audio, model_size, device, compute_type, language, beam_size,
                     ...
                     enable_duration_split, max_duration, enable_char_split, max_chars,
                     enable_silence_split, max_silence_ms, enable_punc_split, punc_chars,
                     force_preprocess, progress=gr.Progress()):
    if audio is None:
        return "请上传音频文件", "", ""
    audio_file, original_name = normalize_file_input(audio)
    ...
    progress(0, desc="初始化...")
    try:
        ensure_model_loaded(model_size, device, compute_type)
    except RuntimeError as e:
        return str(e), "", ""
    progress(0.2, desc="预处理音频...")
    task_sink = []
    audio_path = manager._prepare_audio(audio_file, force_preprocess=force_preprocess, sink=task_sink)
    if not audio_path:
        return "音频处理失败", "", ""
    try:
        ...
        vad_params = None
        if vad_filter:
            vad_params = {"onset": vad_onset, "offset": vad_offset,
                          "min_speech_duration_ms": vad_min_speech,
                          "min_silence_duration_ms": vad_min_silence}
        result, err = manager.transcribe_with_segments(
            audio_path, language=lang, beam_size=beam_size, vad_filter=vad_filter,
            vad_parameters=vad_params, word_timestamps=True, initial_prompt=prompt,
            progress=progress, progress_base=0.2, progress_span=0.4, sink=task_sink)
        if err:
            return f"错误: {err}", "", ""
        notes = _build_split_notes(result, enable_align)
...
def transcribe_video(video, model_size, device, compute_type, language, beam_size,
    ...
    try:
        if video is None:
            return "请上传视频文件", "", ""
        video_path, original_name = normalize_file_input(video)
        if not video_path or not os.path.exists(str(video_path)):
            return "无法获取视频路径", "", ""
        if not original_name:
            original_name = os.path.basename(str(video_path))
        progress(0, desc="初始化...")
        try:
            ensure_model_loaded(model_size, device, compute_type)
        except RuntimeError as e:
        ...
        result, err = manager.transcribe_with_segments(
            audio_path, language=lang, beam_size=beam_size, vad_filter=vad_filter,
            vad_parameters=vad_params, word_timestamps=True, initial_prompt=prompt,
            progress=progress, progress_base=0.4, progress_span=0.2, sink=task_sink)
        if err:
            return f"识别失败: {err}", "", ""
        notes = _build_split_notes(result, enable_align)
        if enable_align and not result.get("split_used"):
        ...
        lang_code_map = {"zh": "chi", "en": "eng", "ja": "jpn", "ko": "kor",
...
def transcribe_batch(files, model_size, device, compute_type, language, beam_size,
    ...
    # ---------- 双缓冲流水线（修复版）----------
    # 关键修复：临时文件按任务隔离登记（sink）。原实现共用 manager.temp_files
    # 全局列表，当前文件推理完成后的清理会把后台已预处理的下一个文件的音频
    # 一并删除，导致从第 2 个文件起必然"转写失败"。现改为每个文件独立 sink，
    # 清理时只删自己的临时文件。
    def prep_task(fp, sink):
        return manager._prepare_audio(fp, force_preprocess=force_preprocess, sink=sink)

    with ThreadPoolExecutor(max_workers=1) as executor:
        def submit_prep(idx):
            fp, _ = normalize_file_input(files[idx])
            sink = []
            if not fp:
                return None, sink
            return executor.submit(prep_task, fp, sink), sink

        future_prep, cur_sink = submit_prep(0)
        for i, fobj in enumerate(files):
            # 1. 获取当前文件的预处理结果（后台已完成或正在完成）
            audio_path = None
            if future_prep is not None:
                try:
                    audio_path = future_prep.result()
                except Exception as e:
            ...
            if i + 1 < len(files):
                ...
                future_prep, next_sink = None, []
            # 3. 处理当前文件
            try:
                if not audio_path:
                    failed_text.append(f"❌ {orig_name}: 预处理失败")
                    continue
                result, err = manager.transcribe_with_segments(
                    audio_path, language=lang, beam_size=beam_size,
                    vad_filter=vad_filter, vad_parameters=vad_params,
                    word_timestamps=True, initial_prompt=prompt, sink=cur_sink)
                if err:
                ...
                # 只清理当前文件登记的临时文件；下一个文件的由其归属迭代负责
                _clean_sink(cur_sink)
                cur_sink = next_sink
                # 注意：不要在此调用 torch.cuda.empty_cache()，会拖慢流水线吞吐
...
def load_model_click(model_size, device, compute_type):
    success, msg = manager.load_asr_model(model_size, device, compute_type)
    return msg + "\n" + get_system_info()
...
# ==================== 界面 ====================
LANG_CHOICES = ["auto", "zh", "en", "ja", "ko", "fr", "de", "es", "pt", "it",
                "ru", "ar", "hi", "th", "vi", "id", "tr", "nl", "uk", "pl",
                "sv", "cs", "ro", "el", "he", "da", "no", "fi", "hu"]
...
def create_interface():
    ...
    with config_lock:
        ...
        OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        if "max_output_length" in settings:
            current_max_output_length = int(settings["max_output_length"])
    ...
    with gr.Blocks(title="WhisperX 语音识别增强版", theme=gr.themes.Default()) as demo:
        gr.Markdown("# 🎤 WhisperX 语音识别 (最终修复版 + 长音频优化)\n输出目录: `{}`".format(OUTPUT_DIR))
        with gr.Accordion("⚙️ 全局设置 (模型/文本截断)", open=True):
            ...
            with gr.Row():
                ...
                max_text_len = gr.Slider(label="界面最大显示字符数", minimum=1000, maximum=20000,
                                         value=current_max_output_length, step=1000,
        ...
        with gr.Accordion("📊 系统状态", open=False):
            status_display_ctrl = gr.Textbox(label="状态", value=get_system_info(),
                                             lines=6, interactive=False)
        with gr.Row():
            open_output_btn = gr.Button("打开输出目录", variant="secondary")
        load_btn.click(load_model_click, inputs=[model_size, device, compute_type],
                       outputs=[status_display_ctrl])
        ...
        def build_split_controls():
            with gr.Accordion("✂️ 断句控制", open=False):
                ...
                with gr.Row():
                    ...
                    silence_slider = gr.Slider(label="最大静音间隔 (毫秒)", minimum=100, maximum=2000,
                                               value=500, step=50, info="单词/字之间超过此静音时长即在此处断开")
                with gr.Row():
                    enable_punc = gr.Checkbox(label="启用标点断句", value=False)
                    punc_text = gr.Textbox(label="断句标点", value="。！？!?……，,;；：:",
                                           placeholder="输入用于断句的标点")
            return [enable_duration, duration_slider, enable_chars, chars_slider,
                    enable_silence, silence_slider, enable_punc, punc_text]

        def build_vad_controls(prefix):
            with gr.Accordion("VAD 高级设置", open=False):
                vad_enable = gr.Checkbox(label="启用 VAD 过滤", value=False)
                vad_onset = gr.Slider(label="语音起始阈值 (onset)", minimum=0.0, maximum=1.0, value=0.6, step=0.05)
                vad_offset = gr.Slider(label="语音结束阈值", minimum=0.0, maximum=1.0, value=0.4, step=0.05)
            ...
            return [vad_enable, vad_onset, vad_offset, vad_min_speech, vad_min_silence]
            ...
            with gr.Tab("音频识别"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        audio_preview = gr.Audio(label="🎧 音频预览（上传后可试听）",
                        ...
                        vad_ctrls_audio = build_vad_controls("audio")
                        enable_align_audio = gr.Checkbox(label="使用 wav2vec2 精细对齐",
                                                         value=False,
                                                         interactive=WHISPERX_ALIGN_AVAILABLE)
                        align_model_audio = gr.Dropdown(label="对齐模型", choices=align_options,
                                                        value="auto", visible=False)
                        enable_align_audio.change(toggle_align_controls, inputs=[enable_align_audio],
                                                  outputs=[align_model_audio])
                        with gr.Row():
                ...
                c_btn.click(lambda: [None, None, "", "", "", ""],
                            outputs=[audio_input, audio_preview, hotwords_audio,
                                     text_out, json_out, srt_out])

            with gr.Tab("视频字幕"):
                with gr.Row():
                    with gr.Column(scale=1):
                        gr.Markdown("### 视频处理\n> 音频会自动转为 16kHz 单声道；超 30 分钟长视频自动分段转写")
                        ...
                        vad_ctrls_video = build_vad_controls("video")
                        enable_align_video = gr.Checkbox(label="使用 wav2vec2 精细对齐",
                                                         value=False,
                                                         interactive=WHISPERX_ALIGN_AVAILABLE)
                        align_model_video = gr.Dropdown(label="对齐模型", choices=align_options,
                                                        value="auto", visible=False)
                        enable_align_video.change(toggle_align_controls, inputs=[enable_align_video],
                                                  outputs=[align_model_video])
                        with gr.Row():
                ...
                vc_btn.click(lambda: [None, "", "", "", ""],
                             outputs=[video_input, hotwords_video, v_text, v_json, v_srt])

            with gr.Tab("批量处理"):
                with gr.Row():
                    with gr.Column(scale=1):
                        files_input = gr.Files(label="上传多个音频",
                                               file_types=[".wav", ".mp3", ".m4a", ".flac", ".ogg"],
                                               file_count="multiple")
                        ...
                        vad_ctrls_batch = build_vad_controls("batch")
                        enable_align_batch = gr.Checkbox(label="使用 wav2vec2 精细对齐",
                                                         value=False,
                                                         interactive=WHISPERX_ALIGN_AVAILABLE)
                        align_model_batch = gr.Dropdown(label="对齐模型", choices=align_options,
                                                        value="auto", visible=False)
                        enable_align_batch.change(toggle_align_controls, inputs=[enable_align_batch],
                                                  outputs=[align_model_batch])
                        with gr.Row():
                            bt_btn = gr.Button("批量识别", variant="primary")
                            bc_btn = gr.Button("清空", variant="secondary")
                    with gr.Column(scale=2):
                        batch_out = gr.Textbox(label="结果", lines=10)
                bt_btn.click(
                    transcribe_batch,
                ...
                bc_btn.click(lambda: [None, "", ""],
                             outputs=[files_input, hotwords_batch, batch_out])
        ...
        def set_max_length(val):
            ...
            with config_lock:
                ...
                merged["max_output_length"] = int(val)
