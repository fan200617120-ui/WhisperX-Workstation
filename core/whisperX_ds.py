...
# -*- coding: utf-8 -*-
"""
WhisperX 语音识别独立增强版（参数全开放 / 修复版）
- 音频识别 / 视频字幕 / 批量处理
- 可选精细时间戳对齐
- 断句控制（时长/字符/标点/静音，可独立组合）
- VAD 高级参数、界面截断保护、一键打开输出目录
- 新增：长音频自动切分（30分钟阈值，10分钟片段）
- 新增：批量处理双缓冲流水线（CPU预处理与GPU推理重叠）
...
def clean_cjk_spaces(text: str) -> str:
    """修复：只删除 CJK 字符之间的空格，保留中英混排时英文单词间的空格。"""
...
def get_system_info():
    ...
    # 新增：显示长音频切分状态
    info.append("长音频切分: 已启用 (>30分钟自动分段)")
    return "\n".join(info)
...
class WhisperXManager:
    ...
    def _split_audio_by_duration(self, audio_path, segment_minutes=10):
        """将长音频切分为临时小段，返回临时文件路径列表"""
        ...
        num_segments = max(1, int(math.ceil(total_duration / segment_duration)))
        temp_files = []
        base_name = Path(audio_path).stem
        for i in range(num_segments):
            start_time = i * segment_duration
            # 生成临时文件（放在 cache 目录）
            temp_file = CACHE_DIR / f"{base_name}_part_{i:03d}.wav"
            if temp_file.exists():
                try:
                    temp_file.unlink()
                except Exception:
    ...
    def transcribe_with_segments(self, audio_path, language=None, beam_size=5,
                                 ...
                                 force_no_split=False, progress=None):
        """
        增强版转写：自动处理长音频切分（对外统一接口）
        """
        if force_no_split:
        ...
        need_split = False
        total_duration = self.get_audio_duration(audio_path)
        if total_duration is not None and total_duration >= 30 * 60:
            need_split = True
        ...
        # 切分
        part_files = self._split_audio_by_duration(audio_path, segment_minutes=10)
        ...
        # 切分粒度固定 10 分钟，偏移必须对齐到切分点（-ss 起点），避免用尾词时间导致时间戳漂移
        segment_duration = 10 * 60

        for idx, part_path in enumerate(part_files):
            if progress is not None and hasattr(progress, 'update'):
                progress((idx+1)/total_parts, desc=f"转写片段 {idx+1}/{total_parts}")
            result, err = self.transcribe(part_path, language, beam_size, vad_filter,
                                          vad_parameters, word_timestamps, initial_prompt)
            if err or not result:
                logger.warning(f"分段 {part_path} 转写失败: {err}")
                continue
            # 固定偏移：第 idx 段对应原音频 [idx*segment_duration, ...)，与 ffmpeg -ss 切分点对齐
            ...
            if result.get("segments"):
                full_segments.extend(result["segments"])
            if result.get("words"):
                full_words.extend(result["words"])
            # 更新语言（取概率最高的）
...
def _norm_lang(language):
    """修复：支持下拉 auto（None 交给 faster-whisper 自动检测）。"""
...
def transcribe_batch(files, model_size, device, compute_type, language, beam_size,
    ...
    # ---------- 双缓冲流水线 ----------
    def prep_task(fp):
        return manager._prepare_audio(fp, force_preprocess=force_preprocess)

    with ThreadPoolExecutor(max_workers=1) as executor:
        # 预先提交第一个文件的预处理
        first_fp, first_orig = normalize_file_input(files[0])
        future_prep = executor.submit(prep_task, first_fp)
        processed_count = 0

        for i, fobj in enumerate(files):
            # 1. 获取当前文件的预处理结果
            audio_path = future_prep.result()
            orig_name = normalize_file_input(fobj)[1] or f"file_{i}"
            ...
            if not audio_path:
                ...
                # 提交下一个任务（如果有）
                if i + 1 < len(files):
                    next_fp, _ = normalize_file_input(files[i+1])
                    future_prep = executor.submit(prep_task, next_fp)
                continue

            # 3. 提前提交下一个文件的预处理任务（CPU 开始工作）
            if i + 1 < len(files):
                next_fp, _ = normalize_file_input(files[i+1])
                future_prep = executor.submit(prep_task, next_fp)

            # 4. 执行 GPU 推理（同时 CPU 在后台处理下一个）
            try:
                prompt = hotwords.strip() if hotwords else None
                vad_params = None
                if vad_filter:
    ...
    # 批量结束后统一清理所有临时文件（预处理 wav + 切分 part）。
    # 不能在循环内清理：后台预处理线程可能正在为下一文件生成临时 wav，会被误删导致后续转写失败。
    manager.cleanup_temp()
...
def create_interface():
    ...
    with gr.Blocks(title="WhisperX 语音识别增强版", theme=gr.themes.Default()) as demo:
        gr.Markdown("# 🎤 WhisperX 语音识别 (修复版 + 长音频优化)\n输出目录: `{}`".format(OUTPUT_DIR))
