...
def transcribe_audio(audio, model_size, device, compute_type, language, beam_size,
    ...
    if audio is None:
        return "请上传音频文件", "", ""
    # 修复：统一解析 Gradio 5 FileInfo / dict / str
    ...
    try:
        ...
        result, err = manager.transcribe(audio_path, language=lang, beam_size=beam_size,
...
def transcribe_batch(files, model_size, device, compute_type, language, beam_size,
    ...
    for i, fobj in enumerate(files, 1):
        # 修复：统一解析 Gradio 5 FileInfo 列表
        fp, orig_name = normalize_file_input(fobj)
        if not orig_name:
            orig_name = os.path.basename(str(fp)) if fp else f"file_{i}"
        progress(i / total, desc=f"处理 {i}/{total}: {orig_name}")
        if not fp or not os.path.exists(fp):
            failed_text.append(f"❌ {orig_name}: 无法获取文件路径")
            continue
        ...
        try:
            ...
            if vad_filter:
                vad_params = {"onset": vad_onset, "offset": vad_offset,
                              "min_speech_duration_ms": vad_min_speech,
                              "min_silence_duration_ms": vad_min_silence}
            result, err = manager.transcribe(ap, language=lang, beam_size=beam_size,
                                             vad_filter=vad_filter, vad_parameters=vad_params,
                                             word_timestamps=True, initial_prompt=prompt)
            if err:
                ...
                continue
            if enable_align:
                result = manager.apply_whisperx_align(result, ap, lang, device, align_model)
            split_args = build_split_kwargs(enable_duration_split, max_duration, enable_char_split,
                                            max_chars, enable_silence_split, max_silence_ms,
                                            enable_punc_split, punc_chars)
            full_text, tsjson, srt_text, _ = format_result_to_outputs(result, **split_args)
